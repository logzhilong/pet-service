...
@Component("countReply")
public class CountReplyHandler extends AbstractHandler{
	...
	public void process(ClientRequest clientRequest,HttpServletResponse response) throws Exception {
		...
		try{
			...
			int countReply = stateService.countReply(clientRequest,authenticationToken);
			
			rtn = new Success(true,countReply).toString();
		}catch(Exception e){
			logger.debug("统计回复数量失败  body="+gson.toJson(clientRequest));
			logger.error("countReply : ",e);
