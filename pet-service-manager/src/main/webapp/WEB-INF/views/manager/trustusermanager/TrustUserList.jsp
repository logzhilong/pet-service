		...
		<tbody id="trustuserList">
			<c:forEach items="${pageBean.data }" var="itm" varStatus="idx">
				<tr target="id" rel="${itm.id }">
					<td>${idx.index+1 }</td>
					<td>
						<a  href="${ctx }/manager/trustuser/trustUserDetail.html?id=${itm.userId }"  rel="trustuser001"   target="dialog" max="false" mask="true" title="托管用户管理(${itm.nrootId })" width="500" height="300" close="forum.refresh">${itm.nrootId }
						</a>
					</td>
				</tr>
			</c:forEach>
